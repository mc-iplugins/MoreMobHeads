...
public class EventHandler_1_20_R2 implements CommandExecutor, TabCompleter, Listener {
	...
	String uuidDEF = "00000001-0000-0000-0000-000000000001";
	String textureDEF = "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZGM5YWRlNDU4Y2QzNzdiZTdiZmYyOTFjMGNmODRmM2ExNWY4M2E0NGYzMGUzM2M4ZmVlNjgyZjY0ZjQ4YmRiZCJ9fX0=";
	List<String> loreDEF = Arrays.asList("");
								...
								if (mmh.DropIt(event, ConfigHelper.Double(chanceConfig,
									...
									Drops.add(
											mmh.makeHead( mmh.langName.getString(name.toLowerCase() + "." + dacattype.toLowerCase(), CatHeads.valueOf(dacattype).getName() + "")
													, CatHeads.valueOf(dacattype).getTexture().toString(), CatHeads.valueOf(dacattype).getUUID(), entity.getType(), entity.getKiller() )
											);
								...
								if (!args[1].isEmpty()) {
									...
									if (args[1].equalsIgnoreCase("steve")) {
										...
										if( mainHand.getType() == Material.PLAYER_HEAD ) {
											...
											String texture = skullPDC.get(mmh.TEXTURE_KEY, PersistentDataType.STRING);
											String sound = skullPDC.get(mmh.SOUND_KEY, PersistentDataType.STRING);

											@Nullable ItemMeta meta = mainHand.getItemMeta();
											if (meta == null){
										...
										case "cat":
											mmh.playerGiveOrDropHead(player,
													mmh.makeHeads(
															...
															CatHeads.valueOf(splitmob[1].toUpperCase()).getTexture(), CatHeads.valueOf(splitmob[1].toUpperCase()).getUUID(), EntityType.CAT, number)
													);
											break;
										case "bee":
